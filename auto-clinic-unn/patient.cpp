...
void to_json(nlohmann::json& json, const Patient& patient)
{
    ...
    if (patient.GetType() == Type::MAJOR_PATIENT) {
        const MajorPatient& major = static_cast<const MajorPatient&>(patient);
...
void from_json(const nlohmann::json& json, Patient& patient)
{
    ...
    if (json["type"] == Type::MAJOR_PATIENT) {
        MajorPatient& major = static_cast<MajorPatient&>(patient);
        major._money = json["money"];
        major._type = Type::MAJOR_PATIENT;
    }
}
